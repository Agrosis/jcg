package json.writers {
  ...
  object GenJsReaders extends  {
    ...
    object ItemJsReader extends JsReader[org.company.app.models.Item] {
      ...
      override def read(value: JsValue): Validation[ItemJsReaderError, org.company.app.models.Item] = value match {
        case JsObject((map @ _)) => idExtractor(map).flatMap(((id) => nameExtractor(map).map(((name) => org.company.app.models.Item(id = id, name = name)))))
    ...
    object PhoneNumberJsReader extends JsReader[org.company.app.models.PhoneNumber] {
      ...
      override def read(value: JsValue): Validation[PhoneNumberJsReaderError, org.company.app.models.PhoneNumber] = value.as[String].mapError(((_) => PhoneNumberInvalidJsonType)).map(((f) => org.company.app.models.PhoneNumber(f)))
    };
    object UserJsReader extends JsReader[org.company.app.models.User] {
      ...
      override def read(value: JsValue): Validation[UserJsReaderError, org.company.app.models.User] = value match {
        case JsObject((map @ _)) => idExtractor(map).flatMap(((id) => usernameExtractor(map).flatMap(((username) => passwordExtractor(map).flatMap(((password) => emailExtractor(map).flatMap(((email) => itemsExtractor(map).map(((items) => org.company.app.models.User(id = id, username = username, password = password, email = email, items = items)))))))))))
