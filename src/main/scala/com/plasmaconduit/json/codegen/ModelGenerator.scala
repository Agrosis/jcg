...
object ModelGenerator {
  ...
  private def getTypeParameters(typeParameters: List[Tree]): Seq[ModelFieldType] = {
    typeParameters.map{
      case Ident(TypeName(name)) => ModelFieldType(name, Seq())
  ...
  private def getFields(body: List[Tree]): List[ModelField] = body match {
    case ValDef(modifiers, TermName(termName), Ident(TypeName(typeName)), body) :: xs => {
      ModelField(ModelFieldTerm(termName), ModelFieldType(typeName, Seq())) :: getFields(xs)
    }
    case ValDef(modifiers, TermName(termName), AppliedTypeTree(Ident(TypeName(typeName)), typeParameters), body) :: xs => {
  ...
  private def traverseForModels(ast: Tree): Seq[Model] = {
    ast match {
      ...
      case ClassDef(modifiers, TypeName(name), typeDef, Template(parents, self, body)) => {
        Seq(Model(ModelName(name), getFields(body), checkIfParentExists(parents, "GenReader"), checkIfParentExists(parents, "GenWriter")))
      }
      case _ => Seq()
    }
  }

  def generateModelsFor(code: String): Seq[Model] = {
